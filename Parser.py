...
def dateCnvTo (dataframe):
    ...
    return(dataframe)
#----
startt = datetime.now(timezone('GMT')).replace(hour = 14, minute = 30, second = 0) #Setting NYSE opening time.
...
for tick in tickers:
    ...
    if df.empty == False :
        df = dateCnvTo(df)
        dir_path = os.path.dirname(os.path.realpath(__file__)) + '\Data\Articles\%s.csv' % (tick)
        df.to_csv(dir_path)
        print(tick + ' Added.')
